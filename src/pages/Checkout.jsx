...
const Checkout = () => {
  ...
  const handleReviewOrder = async () => {
    try {
      ...
      const res = await api.post('orders/order_review', reviewBody, { headers: { Token: `Bearer ${token}` } });
  ...
  const placeOrder = async () => {
    ...
    try {
      ...
      const body = {
        ...
        // Include review data if available to ensure exact order details
                  ...
                  <div className="space-y-3">                    
                    ...
                    {(orderReviewData.order_items || []).map((item, index) => (
                      <div key={index} className="flex justify-between text-sm border-b py-2">
                        <div>{item.product_name_with_variant || item.product_name} × {item.quantity}</div>
                        <div>₹{parseFloat(item.total_price).toLocaleString('en-IN')}</div>
                      </div>
                    ))}
                    ...
                    <div className="mt-4 space-y-2 text-sm">
                      <div className="flex justify-between">
                        <span>Subtotal</span>
                        <span>₹{parseFloat(orderReviewData.subtotal || 0).toLocaleString('en-IN')}</span>
                      </div>
                      {orderReviewData.discount_amount > 0 && (
                        <div className="flex justify-between text-green-600">
                          <span>Discount</span>
                          <span>-₹{parseFloat(orderReviewData.discount_amount).toLocaleString('en-IN')}</span>
                        </div>
                      )}
                      <div className="flex justify-between">
                        <span>Tax</span>
                        <span>₹{parseFloat(orderReviewData.tax_amount || 0).toLocaleString('en-IN')}</span>
                      </div>
                      <div className="flex justify-between">
                        <span>Shipping</span>
                        <span>{orderReviewData.shipping_amount === 0 ? 'FREE' : `₹${parseFloat(orderReviewData.shipping_amount || 0).toLocaleString('en-IN')}`}</span>
                      </div>
                      <div className="pt-2 border-t flex justify-between font-semibold text-lg">
                        <span>Total</span>
                        <span>₹{parseFloat(orderReviewData.total_amount || 0).toLocaleString('en-IN')}</span>
                      </div>
                    </div>
                  </div>
                ) : (
